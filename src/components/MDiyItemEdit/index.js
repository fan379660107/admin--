...
export default class MDiyItem extends Component {
  ...
  * */
  onChange = (val, type, tar_index = 0,parent_index=-1) => {
    let { select_data } = this.props;
    if ((select_data.type == 'lunbo') || (select_data.type == 'nav' && (type != 'icon_set' && type != 'style_set' && type != 'slide')) || select_data.type == 'tupianzuhe' || (select_data.type == 'more_tab' && type != 'border_radius') || (select_data.type == 'top_cat_nav' && type != 'swiper_bg_style')) {
      ...
      this.saveCurSelData(val, type, tar_index, 'more',parent_index);
      return false;
    }
  ...
  saveCurSelData = (val, type, tar_index = 0, flag,parent_index=-1) => {
    let { select_data } = this.props;
    if (flag == 'single') {
      ...
      let tar_data = [];
      if(parent_index>-1){
        if(select_data.type == 'tupianzuhe'&&select_data.sele_style == 8){
          tar_data = select_data.data[parent_index].img.filter((item, index) => index == tar_index)[0];
        }
      }else{
    ...
    if (data.url_type == '') {
      ...
      render_con = <div className={`${styles.sub_part}`} style={{ borderBottomWidth: 0 }}>
        ...
        <FormItem
          ...
          label={''}
        >
          <Input style={{ width: 300 }} maxLength={250} defaultValue={data.url} placeholder={'请输入链接地址'}
      ...
      render_con = <div className={`${styles.sub_part}`} style={{ borderBottomWidth: 0 }}>
        ...
        <FormItem
          ...
          label={''}
        >
          <Input maxLength={250} style={{ width: 300 }} value={data.info.seckillName} disabled={true}/>
        </FormItem>
      </div>;
      ...
      render_con = <div className={`${styles.sub_part}`} style={{ borderBottomWidth: 0 }}>
        ...
        <FormItem
          ...
          label={''}
        >
          <Input maxLength={250} style={{ width: 300 }} value={data.info.videoName} disabled={true}/>
        </FormItem>
      </div>;
    } else if (data.url_type == 'draw') {
      render_con = <div className={`${styles.sub_part}`} style={{ borderBottomWidth: 0 }}>
        <div className={`${styles.subtitle}`}>活动名称</div>
        <FormItem
          key={`link_draw_${index}`}
          {...formItemLayoutModal}
          label={''}
        >
          <Input maxLength={250} style={{ width: 300 }} value={data.info.drawName} disabled={true}/>
        </FormItem>
      </div>;
    }
    return render_con;
  };
  //链接选择器选择之后渲染页面不显示label,parent_index为父级的index
  getDetailItemHideLabel = (data, index = 0,parent_index=-1) => {
    let render_con = '';
    const { select_data } = this.props;
    if (data.url_type == '') {
      render_con = null;
    } else if (data.url_type == 'url') {
      render_con = <Input maxLength={250} key={`${select_data.type}_${select_data.id}_url_${parent_index}_${index}`}
                          className={styles.more_link_input}
                          defaultValue={data.url} placeholder={'请输入链接地址'}
                          onChange={(e) => this.onChange(e.target.value, 'url', index,parent_index)}/>;

    } else if (data.url_type == 'keyword') {
      render_con =
        <Input maxLength={15} key={`${select_data.type}_${select_data.id}_keyword_${parent_index}_${index}`}
               className={styles.more_link_input}
               defaultValue={data.url} placeholder={'请输入关键词'}
               onChange={(e) => this.onChange(e.target.value, 'url', index,parent_index)}/>;
    } else if (data.url_type == 'goods') {
      render_con = <Input key={`${select_data.type}_${select_data.id}_goods_${parent_index}_${index}`} className={styles.more_link_input}
                          value={data.info.goodsName} disabled={true} title={data.info.goodsName}/>;
    } else if (data.url_type == 'category') {
      render_con =
        <Input key={`${select_data.type}_${select_data.id}_category_${parent_index}_${index}`} className={styles.more_link_input}
               value={data.info.categoryName} disabled={true} title={data.info.categoryName}/>;
    } else if (data.url_type == 'topic') {
      render_con = <Input key={`${select_data.type}_${select_data.id}_topic${index}`} className={styles.more_link_input}
                          value={data.info.name} disabled={true}/>;
    } else if (data.url_type == 'seckill') {
      render_con =
        <Input key={`${select_data.type}_${select_data.id}_seckill_${parent_index}_${index}`} className={styles.more_link_input}
               value={data.info.seckillName} disabled={true}/>;
    } else if (data.url_type == 'store') {
      render_con = <Input key={`${select_data.type}_${select_data.id}_store_${parent_index}_${index}`} className={styles.more_link_input}
                          value={data.info.store_name} disabled={true}/>;
    } else if (data.url_type == 'voucher') {
      render_con =
        <Input key={`${select_data.type}_${select_data.id}_voucher_${parent_index}_${index}`} className={styles.more_link_input}
               value={data.info.red_title} disabled={true}/>;
    } else if (data.url_type == 'live') {
      render_con = <Input key={`${select_data.type}_${select_data.id}_live_${parent_index}_${index}`} className={styles.more_link_input}
                          value={data.info.live_name} disabled={true}/>;
    } else if (data.url_type == 'svideo') {
      render_con =
        <Input key={`${select_data.type}_${select_data.id}_svideo_${parent_index}_${index}`} className={styles.more_link_input}
               value={data.info.videoName} disabled={true}/>;
    } else if (data.url_type == 'draw') {
      render_con = <Input key={`${select_data.type}_${select_data.id}_draw${index}`} className={styles.more_link_input}
                          value={data.info.drawName} disabled={true}/>;
    }
    return render_con;
  };
  ...
  * parent_index为父级的index
  * */
  sldHandSeleLink = (val, tar_index = 0, parent_index=-1) => {
    let data = {};
    let { select_data } = this.props;
    if (select_data.type == 'lunbo' || select_data.type == 'nav' || select_data.type == 'tupianzuhe' || select_data.type == 'top_cat_nav') {
      if(parent_index>-1){
        if(select_data.type == 'tupianzuhe'&&select_data.sele_style == 8){
          data = select_data.data[parent_index].img.filter((item, index) => index == tar_index)[0];
        }
      }else{
    ...
    this.parent_index = parent_index;//当前操作数据的父级的序号
  ...
  seleSku = (val) => {
    ...
    let data = {};
    if (select_data.type == 'lunbo' || select_data.type == 'nav' || select_data.type == 'tupianzuhe' || select_data.type == 'top_cat_nav') {
      if(this.parent_index!=undefined&&this.parent_index>-1){
        data = select_data.data[this.parent_index].img.filter((item, index) => index == this.cur_index)[0];
      }else{
  ...
  * */
  setImg = (info, tar_index = 0,tar_parent_index=-1) => {
    const { select_data } = this.props;
    let data = { ...select_data };
    ...
    if (info.file.status === 'done') {
      if (select_data.type == 'lunbo' || select_data.type == 'nav' || select_data.type == 'tupianzuhe' || select_data.type == 'top_cat_nav') {
        //轮播
        let target_data = [];
        if(tar_parent_index > -1){
          if(select_data.type == 'tupianzuhe'&&select_data.sele_style == 8){
            target_data = data.data[tar_parent_index].img.filter((item, index) => index == tar_index)[0];
          }
        }else{
  ...
  * */
  renderTPZH8Img = (item, index, upload_img_tip = '',parent_index=-1) => {
    let limintLength = 15;//限制内容长度
    const { select_data } = this.props;
    //标题的label  标题的key 标题的placeholder
    let title_key = 'title';
    let title_placeholder = '请输入图片标题';
    let content = <Fragment key={`${select_data.type}_${select_data.id}_img${index}`}>
      <div className={`${global.flex_com_row_start_center} ${styles.sld_com_img}`}>
        <div className={`${styles.common_img_part} ${global.flex_com_column_center_center}`}>
          <div className={`${styles.upload_img} ${global.flex_column_center_center}`}>
            <Upload
              ...
              onChange={(info) => this.setImg(info, index,parent_index)}
              headers={{
        ...
        <div className={`${styles.img_con} ${global.flex_com_column_center_flex_start}`}>
          <Input maxLength={limintLength} className={styles.title} placeholder={title_placeholder}
                 onChange={(e) => this.onChange(e.target.value, title_key, index,parent_index)} value={item[title_key]}/>
          <Select
            value={item.url_type}
            style={{ width: 232 }}
            placeholder={'请选择链接类型'}
            onSelect={(e) => this.sldHandSeleLink(e, index,parent_index)}
          ...
          {this.getDetailItemHideLabel(item, index,parent_index)}
        </div>
      </div>
    </Fragment>;
    return content;
  ...
  * upload_img_tip 上传图片的提示
  * */
  renderLunBo = (item, index, upload_img_tip = '',parent_index=-1) => {
    let limintLength = 15;//限制内容长度
    const { select_data } = this.props;
          ...
          <Fragment>
            <div className={`${global.flex_row_start_center}`} style={{ marginBottom: 8 }}>
              ...
              <span className={`${styles.selected_color} ${item.bg_color ? styles.bg_color_current : null}`}
                    ...
                    onClick={() => this.showTopCatNavColorPicker(true, index)}></span>
            </div>
        ...
        <div className={`${styles.selected_svideo_list}`}>
          {data.map((svideo_item, svideo_index) => {
            return <div className={`${styles.selected_svideo_item} ${global.flex_row_start_center}`}>
              ...
              <div onClick={() => this.delSvideo(svideo_item.videoId, tar_index)}
                   className={`${styles.operate} ${global.flex_row_center_center}`}>
                <ALibbSvg fill={'#2d2d2d'} width={18} height={18} type={'shanchu5'}/>
              </div>
            </div>;
        ...
        <div className={`${styles.selected_svideo_list}`}>
          {data.map((svideo_item, svideo_index) => {
            return <div className={`${styles.selected_svideo_item} ${global.flex_row_start_center}`}>
              ...
              <div onClick={() => this.delLive(svideo_item.liveId, tar_index)}
                   className={`${styles.operate} ${global.flex_row_center_center}`}>
                <ALibbSvg fill={'#2d2d2d'} width={18} height={18} type={'shanchu5'}/>
              </div>
            </div>;
  ...
  renderItem = (data) => {
    ...
    if (data.type == 'top_cat_nav') {
      ...
      con = <Fragment>
        <div className={`${styles.sub_part}`}>
          ...
          <FormItem
            ...
            label={''}
          >
            <Radio.Group defaultValue={data.swiper_bg_style}
                         onChange={(e) => this.onChange(e.target.value, 'swiper_bg_style')}>
      ...
      con = <Fragment>
        <div className={`${styles.sub_part}`} style={{ borderBottomWidth: 0 }}>
          ...
          >
            <div className={global.flex_com_column_start_start}>
              {data.data.length > 0 && data.data.map((item, index) => {
                return this.renderLunBo(item, index, '宽710,高不限制');
              })}
              <div className={`${styles.add_goods_wrap} ${global.flex_row_center_center}`}
      ...
      con = <Fragment>
        ...
        <Fragment>
          {data.data.length>0&&data.data.map((item, index) => {
            return <div key={index} className={`${styles.sub_part}`} style={{ borderBottomWidth: 0 }}>
              <div className={`${styles.subtitle}`}>{index%2==0?'左':'右'}{index<2?1:2}模块数据</div>
              <div className={`${global.flex_column_start_start}`}>
                <FormItem
                  key={'tupianzuhe8_main'+index}
                  label={''}
                >
                  <Input maxLength={6} style={{ width: 300 }} placeholder={'请输入主标题,最多6个字'}
                         onChange={(e) => this.onChange(e.target.value,'main_title', index)} value={item.main_title}/>
                </FormItem>
                <FormItem
                  key={'tupianzuhe8_sub'+index}
                  label={''}
                >
                  <Input maxLength={10} style={{ width: 300 }} placeholder={'请输入子标题,最多10个字'}
                         onChange={(e) => this.onChange(e.target.value, 'sub_title', index)} value={item.sub_title}/>
                </FormItem>
                <FormItem
                  key={'tupianzuhe8_url'+index}
                  label={''}
                >
                  <Select
                    value={item.url_type}
                    ...
                    getPopupContainer={triggerNode => triggerNode.parentNode}
                  >
                    {m_diy_link_type().map((item_link, index_link) =>
                      <Option key={index_link} value={item_link.key}>{item_link.name}</Option>,
                    )}
                  </Select>
                </FormItem>
                {this.getDetailItem(item,index)}
              </div>
              <FormItem
                key={'nav_img'}
                label={''}
              >
                <div className={global.flex_com_column_start_start}>
                  {item.img.length > 0 && item.img.map((child, child_index) => {
                    let upload_tip = '宽154*高188';
                    return this.renderTPZH8Img(child, child_index, upload_tip,index);
                  })}
                </div>
              </FormItem>
            </div>
                ...
                <div className={`${styles.selected_svideo_list}`}>
                  {data.data.info.map((svideo_item, svideo_index) => {
                    return <div className={`${styles.selected_svideo_item} ${global.flex_row_start_center}`}>
                      ...
                      <span style={{ width: 80 }}>{svideo_item.click_num}</span>
                      <div onClick={() => this.delSvideo(svideo_item.videoId)}
                           className={`${styles.operate} ${global.flex_row_center_center}`}>
                        <ALibbSvg fill={'#2d2d2d'} width={18} height={18} type={'shanchu5'}/>
                      </div>
                    </div>;
                ...
                <div className={`${styles.selected_svideo_list}`}>
                  {data.data.info.map((svideo_item, svideo_index) => {
                    return <div className={`${styles.selected_svideo_item} ${global.flex_row_start_center}`}>
                      ...
                      <span style={{ width: 80 }}>{svideo_item.viewingNum}</span>
                      <div onClick={() => this.delLive(svideo_item.liveId)}
                           className={`${styles.operate} ${global.flex_row_center_center}`}>
                        <ALibbSvg fill={'#2d2d2d'} width={18} height={18} type={'shanchu5'}/>
                      </div>
                    </div>;
            ...
            <div className={`${styles.add_svideo_wrap} ${global.flex_row_start_center}`}>
              ...
              >
                {getFieldDecorator('more_tab_data_type_' + more_tab_index, { initialValue: more_tab_item.data_type })(
                  <Radio.Group onChange={(e) => this.onChange(e.target.value, 'data_type', more_tab_index)}
                               style={{ marginLeft: 10 }}>
