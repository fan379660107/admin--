...
export default class SldDiyMoreImgModal extends Component {
			...
			columns: [{
              ...
              limit = this.props.extra[record.type+'_limit'];
                  ...
                  maxLength={limit}
					...
					} else if (record.type == 'goods' || record.type == 'category' || record.type == 'topic' || record.type == 'seckill' || record.type == 'draw') {
						content = <div>
			...
			for (let i in ope_data) {
				...
				if (ope_data[i].link_value != undefined && ope_data[i].link_value) {
					...
					if (ope_data[i].link_type == 'url') {
						...
						tmp_info_new.required = true;
					} else if (ope_data.link_type == 'draw') {
						tmp_info_new.name = `${sldComLanguage('抽奖活动名称')}`;
						tmp_info_new.required = true;
					}
  ...
  del_img = (index) => {
	...
	render() {
		const { data, columns, sele_index, img_width, img_height, link_type,admin_show_width,admin_show_height } = this.state;
