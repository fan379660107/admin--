        ...
        render: (text, record) => {
          ...
          if (record.type == 'img') {
            content = <div className={styles.modal_img}>
              ...
              <span
								className={styles.modal_tip_color}>{sldComLanguage('此处要求宽度为')}{this.state.img_width}{sldComLanguage('像素、高度')}{this.state.img_height == 0 ? `${sldComLanguage('不限制')}` : this.state.img_height + `${sldComLanguage('像素')}`}{sldComLanguage('的图片；支持格式gif，jpg，png。')}</span>
              <Upload
