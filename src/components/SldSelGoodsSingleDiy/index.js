...
export default class SldSelGoodsSingleDiy extends Component {
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    ...
    if (nextProps.link_type == 'goods' || nextProps.link_type == 'category' || nextProps.link_type == 'topic' || nextProps.link_type == 'seckill' || nextProps.link_type == 'voucher' || nextProps.link_type == 'draw') {
  ...
  get_list = (params, grade = '') => {
    ...
    dispatch({
      ...
      callback: (res) => {
        ...
        if (res.state == 200) {
          if (link_type == 'goods' || link_type == 'topic' || link_type == 'seckill' || link_type == 'voucher' || link_type == 'draw') {
            data = res.data;
          } else if (link_type == 'category') {
            //id为0直接赋值
            if (grade != '') {
