...
import {
  ...
  sldtbaleOpeBtnText, sucTip, sldPopConfirmDiy, failTip, sldPopConfirm,
} from '@/utils/utils';
...
@Form.create()
export default class RankActivityLists extends Component {
  constructor(props) {
    super(props);
    this.state = {
      initLoading: false,
      ...
      search_data: [{
        ...
        type: 'select',
        label: `${sldComLanguage('榜单分类')}`,
        name: 'categoryId',
        placeholder: `${sldComLanguage('请选择榜单分类')}`,
        sel_data: [],
        diy: true,
        sele_key: 'categoryId',
                ...
                :<Fragment>
                  ...
                  <Fragment>
                    <div className={global.rank_sync_icon} style={{ marginLeft: 5 }}
                      ...
                      <Icon title={sldComLanguage('点击更新数据')} type="sync" style={{ color: '#778595' }}/>
                    </div>
                  </Fragment>
                  }
                </Fragment>
  ...
  componentDidMount() {
    ...
    this.get_cat_list({ pageSize: list_com_page_more });
  }
  ...
  get_cat_list = (params) => {
    ...
    dispatch({
      ...
      callback: (res) => {
        if (res.state == 200) {
          let temp = search_data.filter(item => item.name == 'categoryId');
          res.data.unshift({ categoryId: '', categoryName: `${sldComLanguage('全部')}` });
          temp[0].sel_data = res.data;
        }
  ...
  //新建排行榜
  add = () => {
    router.push('/marketing_promotion/rank_to_add');
  };
    ...
    return (
      <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
        ...
        <div className={global.operate_bg}>
          {sldIconBtn(() => this.add(), `${sldComLanguage('新建排行榜')}`, 7, 0, 14, 14, 3, 'fabu1', '#FA6F1E')}
          {sldPopConfirm('leftBottom', `${sldComLanguage('关闭全部榜单后用户将无法看到，确认关闭吗')}？`, () => this.operate('','close'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`, sldIconBtn(null, `${sldComLanguage('关闭全部榜单')}`, 7, 0, 15, 15, 4, 'guanbisuoyou', '#fa0920'), 0, 0, '#fa0920')}
        </div>
